package com.zs.tools
{
	...
	public class HitTest
	{
		...
		public static function complexHitTestObject(target1:DisplayObject, target2:DisplayObject):Boolean
		{
			...
			var scaleX:Number = (target1.width < target2.width ? target1.width : target2.width) / HitTest.tileSize 
			//纵向缩小到tileSize尺寸需要的倍
			var scaleY:Number = (target1.height < target2.height ? target1.height : target2.height) / HitTest.tileSize 
			//
			...
			return complexIntersectionRectangle(target1, target2 , scaleX , scaleY , pt , ct , oldHitRectangle,hitRectangle,HitTest.tileSize,HitTest.tileSize).width != 0;
		}		
		...
		{		
			...
			bitmapData.draw(target1, HitTest.getDrawMatrix(target1, hitRectangle , scaleX , scaleY ),ct);
			...
			bitmapData.draw(target2, HitTest.getDrawMatrix(target2, hitRectangle , scaleX , scaleY ),ct, BlendMode.ADD);
			...
			if(intersection.width!=0){				
				...
				if(scaleX>1||scaleY>1){
					...
					if(hits<=(scaleX+scaleY)*1.5){
						...
						var nextW:int=HitTest.tileSize
						var nextH:int=HitTest.tileSize
						...
						if(intersection.width!=nowW){
							nextW=HitTest.tileSize					
						}else{
							nextW=nowW*2
						}				
						if(intersection.height!=nowH){
							nextH=HitTest.tileSize
						}else{
							nextH=nowH*2
						}
